...
import { getWatchlist, getWatchHistory, removeFromWatchlist, setWatchlistCategory } from '../services/userStore';
import { WatchlistItem, WatchProgress, WatchlistCategory } from '../types/user';
...
export default function WatchlistPage() {
  ...
  const [activeTab, setActiveTab] = useState<'watching' | 'completed' | 'plan_to_watch' | 'on_hold' | 'dropped' | 'history'>('watching');
    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-900 pb-6">
        <div>
          ...
          <p className="text-xs text-slate-400">Manage saved anime categories and watch history timestamps.</p>
        </div>
          ...
          {history.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {history.map((item) => (
                ...
                >
                  <img src={item.coverImage} alt={item.title} className="w-16 h-20 object-cover rounded-xl shrink-0" />
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="w-full bg-slate-900 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div className="bg-purple-500 h-full rounded-full" style={{ width: `${item.percentage || 10}%` }} />
                    </div>
                    <Link
